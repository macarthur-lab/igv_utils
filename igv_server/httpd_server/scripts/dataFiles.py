...
BASE_URL = "http://%(server_name)s:%(server_port)s" % locals()
...
#log("URL ARGS: " + str(url_args))
if not url_args:
    #print("""<b>ERROR:</b> 'genome' and 'directory' URL parameters not provided.<br><br>
    #URL must be of the form: %(BASE_URL)s/scripts/dataFiles.py?genome=hg19&directory=/some/directory/with/files""" % locals())
...
sort_files = url_args_dict.get('sort', [''])[0]
if not directory:
...
def create_resources_xml(path, subpath="", level=0):
    ...
    listdir = os.listdir(os.path.join(path, subpath))
    if sort_files == "alphabetical":
        listdir.sort()
